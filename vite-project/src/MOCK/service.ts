...
import { productsDB, categories} from './data.ts';
...
export const productService = {
  ...
  async getAllProducts(options = {}): Promise<Products[]> {
    await delay(200);
  ...
  async getProductsByCategory(category: string, options = {}): Promise<Products[]> {
    await delay(150);
    let products =
      category === 'all'
        ? [...productsDB]
  ...
  async searchProducts(query: string, options = {}): Promise<Products[]> {
    await delay(200);
    let products = !query
      ? [...productsDB]
      : productsDB.filter(
  ...
  applyFiltersAndSort(products: Products[], options = {}) {
    const {
      filters = {},
      sortBy = 'relevance',
      category = 'all',
  ...
  sortProducts(products: Products[], sortBy: string ) {
